...
- **The M4.5 plan document is inconsistent with the delivered state.** `docs/plans/todo/2026-04-19-exact-theorem-4-entropic-region-closure.md` still says `status: proposed` in the front matter and `## Status\n\nNot started.` in the body. Its `depends_on` line references `theorem4_closure`, which no longer exists (renamed). If this branch merges as-is, the follow-up plan will claim work is "Not started" while its verification criteria are all already met. **Recommended fix: move the plan to `docs/plans/done/` and refresh its status/dependency lines in the same edit, mirroring how the M4 plan was moved.**
...
1. **`docs/plans/todo/2026-04-19-exact-theorem-4-entropic-region-closure.md` should move to `docs/plans/done/` with updated status.** Its Verification section's bullets (1)-(5) are all already true on this branch; its Exit criteria are met. Leaving it in `todo/` with `status: proposed` / `## Status: Not started` misrepresents the repo state. Fix in a short doc commit before merge or immediately after: move the file, flip `status` to `done`, and either remove the "Not started" line or replace it with a pointer to the commits that closed the work (`0ff5b28`, `9e36c15`, plus the prior stretch commits that laid the groundwork).
2. **The M4.5 plan's `depends_on` references `theorem4_closure`.** Rename to `theorem4_seqClosure` in the same doc edit. Minor, but avoids a broken pointer.
