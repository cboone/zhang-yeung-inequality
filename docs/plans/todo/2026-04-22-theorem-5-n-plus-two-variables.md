...
- `ZhangYeung/Theorem3.lean` with `zhangYeung` (eq. 21), `zhangYeung_dual` (eq. 22), `zhangYeung_averaged` (eq. 23), and the private `zhangYeung_integer`.
...
- Identical distribution / conditional independence: `IdentDistrib`, `IdentDistrib.comp`, `IdentDistrib.symm`, `IdentDistrib.mutualInfo_eq`, `IdentDistrib.condMutualInfo_eq` (promoted in §6.2.4 below if it stays private in M2), `CondIndepFun`, `condIndep_copies`.
- From the project: `ZhangYeung.delta`, `delta_def`, `delta_self`, `delta_comm_main`, `delta_form21_iff`; `copyLemma`, `delta_of_condMI_vanishes_eq`, the six transport corollaries; `zhangYeung`, `zhangYeung_integer`; `condIndepFun_comp`.
...
- Potential new PFR primitive: `H[⟨B_1, ..., B_n⟩ | A ; μ] ≤ ∑_k H[B_k | A ; μ]` in some form (iterated conditional subadditivity). If not directly available under that shape, derive it locally by induction on `n` from `entropy_triple_add_entropy_le` (see §6.4.3).
...
1. **Copy lemma at a tuple codomain.** Apply PFR's `condIndep_copies` to the pair `(⟨X 0, ..., X (n-1)⟩, ⟨U, Z⟩)`. This gives two conditionally-independent tuple copies `X' : Ω' → (∀ j, S j)` and `X* : Ω' → (∀ j, S j)`, each identically distributed with `⟨X 0, ..., X (n-1)⟩` under the shared `(U', Z')`. All M5 projection measurabilities ("apply `X' j := fun ω => X' ω j`") are one-line `Measurable.eval` / `measurable_pi_apply` invocations.
1. **Pairwise Lemma 2 applied $n$ times.** For each `k : Fin n`: instantiate `delta_of_condMI_vanishes_eq` at `(A, B, C, D) := (X' i, X* k, Z', U')`. The vanishing-CMI hypothesis `I[X' i : X* k | ⟨Z', U'⟩ ; ν] = 0` follows by projecting the tuple-level `CondIndepFun X' X* ⟨Z', U'⟩ ν` to the pair `(i, k)` via `ZhangYeung.condIndepFun_comp`. The resulting inequality is $\Delta(Z', U' \mid X'_{i}, X^{*}_{k}) \le I(X'_{i}; X^{*}_{k})$.
1. **Marginal equality, summed.** Transport the $n$ pairwise delta inequalities back to the $(Z, U)$-side using `IdentDistrib.condMutualInfo_eq` (promoted from M2). Summing over $k$: $n \, I(Z; U) - n \, I(Z; U \mid X_{i}) - \sum_{k} I(Z; U \mid X_{k}) \le \sum_{k} I(X'_{i}; X^{*}_{k})$.
...
1. **Closing marginal equalities.** $I(X'_{i}; \langle Z', U'\rangle) = I(X_{i}; \langle Z, U\rangle)$ via the first-copy triple `IdentDistrib`; $T(X^{*}_{1}, \ldots, X^{*}_{n}) = T(X_{1}, \ldots, X_{n})$ via the second-copy $(n+2)$-tuple `IdentDistrib`.
...
theorem theorem5 ... := by
  ...
  have hUZ : Measurable (fun ω => (U ω, Z ω)) := hU.prodMk hZ
  obtain ⟨Ω', mΩ', Xprime, Xstar, VUZ, ν, _, hXprime, hXstar, hVUZ,
      hCond, hIdent₁, hIdent₂⟩ :=
    condIndep_copies Xtuple (fun ω => (U ω, Z ω)) hXtuple hUZ μ
  -- Project out U', Z' from VUZ and X' j, X* j from Xprime, Xstar via `Measurable.eval`.
  let U' : Ω' → S_U := fun ω => (VUZ ω).1
  let Z' : Ω' → S_Z := fun ω => (VUZ ω).2
  let X' : ∀ j : Fin n, Ω' → S j := fun j ω => Xprime ω j
  let X* : ∀ j : Fin n, Ω' → S j := fun j ω => Xstar ω j
  have hU' : Measurable U' := measurable_fst.comp hVUZ
  have hZ' : Measurable Z' := measurable_snd.comp hVUZ
  ...
  -- tuple-level `CondIndepFun` via `condIndepFun_comp` twice (once on each side).
  have h_vanish : ∀ k : Fin n,
      I[X' i : X* k | (fun ω' => (Z' ω', U' ω')) ; ν] = 0 := fun k => by
    refine (condMutualInfo_eq_zero (hX' i) (hX* k)).mpr ?_
    -- Project `CondIndepFun Xprime Xstar VUZ ν` to coordinate (i, k) on the paired side,
    -- and swap `VUZ` to `(Z', U')` via a measurable pi-to-prod repacking.
    sorry
  have h_pair : ∀ k : Fin n, delta Z' U' (X' i) (X* k) ν ≤ I[X' i : X* k ; ν] := by
    intro k
    -- Apply `delta_of_condMI_vanishes_eq` and drop the three nonneg CMI terms.
    sorry
  ...
  have h_transport : ∀ k : Fin n, delta Z U (X i) (X k) μ = delta Z' U' (X' i) (X* k) ν := by
    intro k
    -- Build triple-level IdentDistribs `(Z, U, X i) ~ (Z', U', X' i)` from `hIdent₁`
    -- and `(Z, U, X k) ~ (Z', U', X* k)` from `hIdent₂`, then expand `delta` via `delta_def`
    -- and `IdentDistrib.condMutualInfo_eq` three times.
    sorry
  ...
  have h_marg_T :
    ...
    -- Tuple-level `IdentDistrib` from `hIdent₂` gives the first component;
    -- projecting to each coordinate gives the second.
    sorry
...
```text
ZhangYeung/
  Prelude.lean              # Promote `mutualInfo_le_of_condIndepFun` and `mutualInfo_add_three_way_identity` here from Theorem3.lean (§7.3).
...
AGENTS.md (aka CLAUDE.md)   # add two Module Layout entries; note the two M3-to-Prelude promotions.
```
...
│     - `tuple_condIndepFun_pairProj` : project `CondIndepFun Xprime Xstar VUZ ν` to a pair (§6.4.2)
...
1. **Bootstrap + pre-flight checks.** In the `m5-theorem-5` worktree: `bin/bootstrap-worktree`; confirm `make check` is green with M0-M4 on `main`. Run two pre-flight experiments in a scratch `.lean` file (delete after):
    ...
    1. **PFR primitives grep.** Confirm `condIndep_copies` still exists at `.lake/packages/PFR/PFR/ForMathlib/ConditionalIndependence.lean:135` and its codomain-polymorphic signature still accepts `α := ∀ j : Fin n, S j` (with the matching `[Countable α]` and `[MeasurableSingletonClass α]` typeclass obligations).
...
1. **Promote two M3 helpers to `ZhangYeung/Prelude.lean`.** `mutualInfo_add_three_way_identity` and `mutualInfo_le_of_condIndepFun` both live `private` inside `ZhangYeung/Theorem3.lean` (§7.3). M5 is the first downstream consumer of either; promote both in a single non-API-breaking refactor commit. Reroute the two call sites in `ZhangYeung/Theorem3.lean` (inside `zhangYeung_integer`) to the promoted public names. `make check` stays green. Commit as `refactor: promote three-way identity and data-processing helpers to Prelude`.
...
1. **Land the `Fin n` projection helpers.** Small measurability lemmas for `Measurable.eval`-style projections of tuple-valued random variables, and one helper for the pi-to-prod repacking of `VUZ` into `(Z', U')`. One commit; ≤ 20 lines. `feat(theorem5): add Fin-indexed projection helpers`.
...
1. **Land `mutualInfo_add_n_way_inequality`.** The $n$-ary chain-rule MI domination. Statement:
    ...
    Proof plan: rewrite the inequality as `H[⟨B 0, ..., B (n-1)⟩ | A ; μ] ≤ ∑ k, H[B k | A ; μ]` (iterated conditional subadditivity), then prove by induction on `n`. The base case `n = 0` is `H[0 | A ; μ] = 0 ≤ 0`; the base case `n = 1` is reflexive; the inductive step uses `entropy_triple_add_entropy_le` (a.k.a. conditional subadditivity, PFR Basic.lean:1117) once. Use `Fin.cases` or `Fin.induction` to step through. Budget ~40 lines, split across ~3 private helper lemmas if the induction proof proves fragile (follow the split-before-bump guideline). Commit: `feat(theorem5): prove n-ary MI chain-rule domination`.

    **Alternative (simpler if it exists in PFR):** if PFR or Mathlib exposes a ready-made iterated conditional subadditivity lemma (something like `entropy_pi_le_sum_entropy` conditioned on `A`), reroute the proof through it. Grep before starting: `find .lake/packages -name "*.lean" -exec grep -l "pi_le_sum_entropy\|sum_cond_entropy\|cond_entropy_pi" {} \;`. If found, use; if not, build locally.

1. **Land `theorem5_integer` as a private theorem.** The main Shannon chase. Follow the six-step body sketched in §6 above. All helper lemmas already in place; this file is linear. Budget ~60 tactic lines. Commit: `feat(theorem5): prove integer form of Theorem 5 via n-fold copy-lemma chase`.

1. **Land `theorem5` as a public theorem.** Thin wrapper rescaling the integer form to match paper eq. (27). ≤ 10 lines. Commit: `feat(theorem5): state Theorem 5 in the paper form (eq. 27)`.
...
1. **Expand `ZhangYeungTest/Theorem5.lean`.** Sequence step 3 landed the signature-pinning `example`s; expand with:
    ...
    - **Independent-variable smoke test.** For `X : Fin 3 → Ω → Fin 2` with the three coordinates and `(U, Z)` all mutually independent, the LHS of eq. (27) is nonpositive and the RHS is nonnegative (all MIs = 0, all cross-entropies sum as if independent). Pin this as an `example`.
...
1. **Update `AGENTS.md` / `CLAUDE.md`.** Two new Module Layout lines (one each for `ZhangYeung/Theorem5.lean` and `ZhangYeungTest/Theorem5.lean`). Update the `ZhangYeung/Prelude.lean` entry to note the two new public helpers. Commit: `docs: document Theorem 5 module in CLAUDE.md`.
...
1. **Open the PR.** Title: `feat: prove Theorem 5, the n+2-variable Zhang-Yeung generalization`. Body links this plan and the roadmap, summarizes `theorem5` and `theorem5_averaged`, and calls out the M3 helper promotions (`mutualInfo_add_three_way_identity`, `mutualInfo_le_of_condIndepFun`) as prerequisite refactors. Note the base-case compatibility `example` as the core correctness check.
...
1. **The `condIndep_copies` tuple specialization.** PFR's `condIndep_copies` is stated at a single pair `(X : Ω → α, Y : Ω → β)` with `[Countable β]`. Specializing $\alpha := \prod_{j} S_{j}$ requires `[Countable β]` to still hold for $\beta = S_{U} \times S_{Z}$, which it does from `[Finite S_U] [Finite S_Z]`. The remaining typeclass obligation `[MeasurableSingletonClass β]` is similarly inherited. **Risk:** the tuple-valued copy's `Xprime, Xstar : Ω' → (∀ j, S j)` may elaborate awkwardly against PFR's `⟨X_1, X_2, Y'⟩` convention. **Mitigation:** rehearse the `condIndep_copies` specialization at step 1 of the sequencing (pre-flight check); if the elaboration is fragile, wrap the tuple codomain with a one-line `let` binding and cast as needed.
1. **The pairwise vanishing-CMI step.** `I[X' i : X* k | ⟨Z', U'⟩ ; ν] = 0` follows from the tuple-level `CondIndepFun Xprime Xstar VUZ ν` by projecting each side via `condIndepFun_comp` with `measurable_pi_apply i` (resp. `measurable_pi_apply k`). The condition variable `VUZ` is itself a pair-valued function `Ω' → S_U × S_Z`; the `I[... | ⟨Z', U'⟩ ; ν]` hypothesis wants the pair `(Z', U')`, so a one-line `IdentDistrib`-style or measurable-repacking lemma is needed to swap `VUZ` for `⟨Z', U'⟩`. **Mitigation:** if the repacking is fragile, inline it as a small private lemma (`measurable_pairZU`-analogue for this file).
...
### 7.3 M3 helper promotion mechanics (low)

`mutualInfo_add_three_way_identity` and `mutualInfo_le_of_condIndepFun` are both `private` in `ZhangYeung/Theorem3.lean:65-104`. M5 consumes both: the first for the generalized $n$-way chain-rule identity (specialized to $n = 2$ for the inductive step of `mutualInfo_add_n_way_inequality`), the second for the data-processing step (step 5 of the chase). Promote both to `ZhangYeung/Prelude.lean` in sequencing step 2 following the same pattern used for `condIndepFun_comp` in M3. Both are generic (no Zhang-Yeung-specific content), and both are good Mathlib-upstream candidates in follow-up work.

**Mitigation:** before sequencing step 2, confirm the only existing call sites are the two uses inside `zhangYeung_integer` (grep: `grep -rn "mutualInfo_add_three_way_identity\|mutualInfo_le_of_condIndepFun" ZhangYeung/ ZhangYeungTest/`). If a second module already depends on them (unexpected but possible), reroute that module in the same commit.
...
This is **iterated conditional subadditivity**. The base case `n = 2` is `entropy_triple_add_entropy_le` (PFR Basic.lean:1117), and the inductive step chains the pair `(B_{1}, \langle B_{2}, \ldots, B_{n}\rangle)`. PFR / Mathlib may or may not already expose this under a usable name.

**Grep first (§6.1, sequencing step 1):**
...
If a usable lemma exists, reroute `mutualInfo_add_n_way_inequality` through it and skip the `Fin n` induction. If not, build locally via `Fin.induction` / `Nat.rec` over $n$. Budget the local build at 40 lines split across ~3 helpers.
...
$$
(X^{*}_{1}, \ldots, X^{*}_{n}, U', Z') \;\sim\; (X_{1}, \ldots, X_{n}, U, Z).
$$

This is exactly what `hIdent₂` from `condIndep_copies` provides, modulo a pi-to-prod encoding and the `VUZ` vs. `⟨U', Z'⟩` repacking. Then the entropy transport `H[⟨X^{*}_{1}, \ldots, X^{*}_{n}\rangle ; \nu] = H[\langle X_{1}, \ldots, X_{n}\rangle ; \mu]` follows from `IdentDistrib.comp` with the identity-up-to-isomorphism projection plus `IdentDistrib.entropy_eq` (or equivalently, `mutualInfo_eq` with the conditioner being a constant).

**Risk:** PFR may not expose `IdentDistrib.entropy_eq` as a one-line lemma; if not, route through `mutualInfo_def` + `IdentDistrib.mutualInfo_eq` applied against a constant function, or through `IdentDistrib.condEntropy_eq` against a trivial conditioner. Confirm in the §6.1 pre-flight audit.

**Mitigation:** if the entropy-level transport proves fragile, expose an auxiliary `IdentDistrib.entropy_eq` helper locally in `ZhangYeung/Prelude.lean` (it is generic and independent of the Zhang-Yeung proof). Treat the auxiliary lemma as a candidate for separate upstream / Mathlib contribution.
...
1. Base-case compatibility `example` per §5 above: Theorem 5 at `n = 2, i = 0` recovers the integer form of Theorem 3 (the shape `delta_form21_iff` converts between). This is a cross-check that the headline statement specializes cleanly.
1. Independent-variable smoke test at `n = 3` and `Fin 2`-valued codomains: with all five random variables mutually independent, the LHS is nonpositive and the RHS is nonnegative.
...
- No upstreaming of `mutualInfo_add_n_way_inequality` or the two M3 helpers to Mathlib. All three are good candidates; defer to roadmap §9 extensions.
...
- `ZhangYeung/Prelude.lean` (modified: add `mutualInfo_add_three_way_identity` and `mutualInfo_le_of_condIndepFun` promoted from `Theorem3.lean`).
...
- `AGENTS.md` / `CLAUDE.md` (modified, two-line addition under "Module Layout"; note on the `Prelude` change).

**Read-only references:**

- `ZhangYeung/Delta.lean` (M1 output).
- `ZhangYeung/CopyLemma.lean` (M2 output; `copyLemma`, `delta_of_condMI_vanishes_eq`, the six transport corollaries; the private `IdentDistrib.condMutualInfo_eq` helper candidate for promotion during sequencing step 5).
