...
public import ZhangYeung.ForMathlib.FiniteRange
public import ZhangYeung.Mathlib.MeasureTheory.Measure.Dirac
public import ZhangYeung.Mathlib.MeasureTheory.Measure.Real
...
* `FiniteSupport`: class expressing that a measure is supported on a finite set.

## Main statements

* `measureEntropy_nonneg`, `measureMutualInfo_nonneg`: nonnegativity of entropy and mutual
  information.
* `measureEntropy_le_log_card`: the entropy of a measure on a finite type is at most the log of the
  cardinality.
* `measureEntropy_map_of_injective`: the entropy is invariant under pushforward along an injective
  map.
* `measureEntropy_prod`: the entropy of a product measure is the sum of the marginal entropies.
* `measureMutualInfo_swap`, `measureMutualInfo_prod`: basic properties of mutual information.
...
* [teorth/pfr](https://github.com/teorth/pfr) at rev `80daaf1` — file
  `PFR/ForMathlib/Entropy/Measure.lean` that this file is forked from, stripped of
  `uniformOn`-specific lemmas not needed for Zhang-Yeung.
* Thomas M. Cover and Joy A. Thomas. *Elements of information theory*. Second edition.
-/
