...
/-!
# Finite range of a function

The `FiniteRange` typeclass expresses that a function `X : Ω → G` has finite range. This is the
primary finiteness mechanism used throughout the extracted Shannon entropy API.

The file is forked from `PFR.ForMathlib.FiniteRange.Defs` in
[teorth/pfr](https://github.com/teorth/pfr) at rev `80daaf1`, with no change in content. It is
kept here until `FiniteRange` is upstreamed to Mathlib.

## Main definitions

* `FiniteRange`: typeclass expressing that `Set.range X` is finite.
* `FiniteRange.toFinset`: the range as a `Finset`.

## References

* [teorth/pfr](https://github.com/teorth/pfr) — original source.
-/
