...
M4 is complete: Theorem 4 of [@zhangyeung1998, §II eq. 26] at `n = 4`, the scientific payoff of the Zhang-Yeung inequality, is formalized as the public `theorem4`. The proof combines (a) a `ℚ`-valued counterexample witness `F_witness_ℚ` on `Finset (Fin 4)` (the paper's explicit set function on lines 368-377, specialized to `a = 1`) discharged by `native_decide` on the Shannon-cone axioms; (b) the specific Zhang-Yeung violation at the canonical labeling `(2, 3, 0, 1)` via the permutation `Equiv.swap 0 2 * Equiv.swap 1 3`; and (c) a set-function bridge `zhangYeungHolds_of_entropy` that restates M3's `zhangYeung` at the set-function level over a heterogeneous family `X : ∀ i : Fin 4, Ω → S i`, invoked once per permutation of `Fin 4`. The module also introduces the set-function calculus `I_F`/`condI_F`/`delta_F` and the cone predicates `shannonCone`/`zhangYeungAt`/`zhangYeungHolds` mirroring paper eqs. 3-5, 11, 21, and 25.
...
- [`ZhangYeung.lean`](ZhangYeung.lean) — project entrypoint; re-exports `ZhangYeung.CopyLemma`, `ZhangYeung.Delta`, `ZhangYeung.Prelude`, `ZhangYeung.Theorem2`, `ZhangYeung.Theorem3`, and `ZhangYeung.Theorem4`.
...
- [`ZhangYeung/Theorem4.lean`](ZhangYeung/Theorem4.lean) — M4 Shannon incompleteness at `n = 4` (Theorem 4 of the 1998 paper, §II eq. 26): the set-function calculus (`I_F`, `condI_F`, `delta_F`), the cone predicates (`shannonCone`, `zhangYeungAt`, `zhangYeungHolds`), the rational-valued counterexample witness (`F_witness_ℚ`, `F_witness`), the heterogeneous four-variable entropy function (`entropyFn`), the per-subset bridge lemmas (`entropyFn_empty`, `entropyFn_singleton`, `entropyFn_pair`, `entropyFn_triple`, `entropyFn_quad`), the permutation-indexed bridge `zhangYeungAt_entropyFn` routed through M3's `zhangYeung`, and the headline `theorem4` separating `Γ_4` from the set of entropy functions of four discrete random variables.
- [`ZhangYeungTest.lean`](ZhangYeungTest.lean) + [`ZhangYeungTest/Delta.lean`](ZhangYeungTest/Delta.lean) + [`ZhangYeungTest/Theorem2.lean`](ZhangYeungTest/Theorem2.lean) + [`ZhangYeungTest/CopyLemma.lean`](ZhangYeungTest/CopyLemma.lean) + [`ZhangYeungTest/Theorem3.lean`](ZhangYeungTest/Theorem3.lean) + [`ZhangYeungTest/Theorem4.lean`](ZhangYeungTest/Theorem4.lean) — compile-time API regression tests for each module.
